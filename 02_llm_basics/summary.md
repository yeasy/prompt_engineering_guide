...
3. **模型参数作用**
   - **Temperature**：调整概率分布的形状（0 = 贪婪解码；<1 锐化；=1 为模型原始分布；>1 摊平分布）
   - **Top-p**：核采样，限制采样范围
   - **Max Tokens**：限制输出长度
   - **惩罚参数**：控制重复程度
...
[第三章](../03_prompt_structure/README.md)将进入提示词设计的实践环节，详细介绍提示词的基本结构，包括核心组成要素、指令设计原则、上下文提供策略和输出格式定义，帮助读者掌握构建有效提示词的基本技能。
