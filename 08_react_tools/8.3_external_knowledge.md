...
在工程实践中，我们通常不会通过爬虫直接抓取 Google 搜索结果（不稳定且违规），而是使用专门的搜索 API：

-   **Google Custom Search / Bing Search API**：官方商业 API，稳定但成本较高。
-   **SerpApi / Serper**：第三方聚合服务，提供 Google/Bing 的结构化 JSON 结果，对开发者友好。
-   **Tavily**：专为 AI Agent 优化的搜索引擎，返回的内容经过清洗，去除了广告和无关干扰，更适合 LLM 阅读。

#### 搜索 Agent 的设计模式

简单的"搜索-回答"往往不够，高级模式包括：

*   **多步搜索 (Multi-step Search)**：
    *   *问题*："2024年诺贝尔物理学奖得主的最新研究成果是什么？"
    *   *Step 1*：搜索 "2024 Nobel Physics Prize winners"（获取人名）。
    *   *Step 2*：搜索 "[Winner Name] latest research papers 2024 2025"。
*   **来源核查 (Fact Checking)**：
    *   要求 Agent 在回答中必须引用搜索结果的 URL。
    *   系统提示词示例：*"对于每一个事实陈述，必须在句末标注来源链接，例如 [1](url)。"*
