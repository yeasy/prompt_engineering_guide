...
在 ReAct 框架中，工具的使用是通过**函数调用** 机制实现的。现代大语言模型（如 GPT-5.x、Claude 4.5、Gemini 3.0）都提供了原生的函数调用能力，这使得将 LLM 连接到外部系统变得更加可靠和结构化。本节将深入探讨函数调用的机制、高级模式及最佳实践。
...
**目的**：约束模型输出为可机器解析的结构化 JSON，减少后处理与格式漂移。
**输入**：JSON Schema/类型定义 + 用户输入；必要时指定 `response_format`/schema 约束参数。
**输出**：模型输出为严格符合 schema 的 JSON；解析失败时可触发重试或纠错提示。
**注意**（可选）：对 schema 做版本化；对不可信字段做校验；记录原始响应便于回放调试。
