...
每个步骤都是独立的模型调用，如果不主动传递，前序步骤的信息会丢失。

#### 信息冗余

传递过多信息会导致Token浪费和注意力稀释。
...
上下文传递和状态管理是提示词链成功运行的关键。应根据任务特点选择合适的传递策略和状态管理模式，定义清晰的步骤接口，并实现健壮的错误处理。对于大量信息传递的场景，可以使用摘要、提取或结构化等压缩技术来优化Token使用。
