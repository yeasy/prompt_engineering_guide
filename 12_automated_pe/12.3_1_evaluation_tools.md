...
from deepeval.metrics import FactualCorrectness, Relevance
...
results = evaluate(
    ...
    metrics=[factual_metric, relevance_metric]
)
...
for result in results:
    ...
    print(f"Factual: {result.metrics['FactualCorrectness']}")
...
| 指标 | 说明 | 使用场景 |
|------|------|---------|
| FactualCorrectness | 检查输出是否包含事实错误 | 知识问答、信息提取 |
| Relevance | 检查是否回答了问题 | 所有对话任务 |
| Hallucination | 检查是否包含虚假信息 | 需要事实准确的应用 |
| Toxicity | 检查有害内容 | 所有面向用户的应用 |
| Bias | 检查是否存在偏见 | 敏感领域（法律、医疗等） |
| Summarization | 评估摘要质量 | 文本摘要任务 |
| AnswerRelevance | 答案与问题的匹配度 | 问答系统 |
