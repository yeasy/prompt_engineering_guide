...
| 指标 | 适用场景 | 计算方式 |
|------|---------|---------|
| 准确率（Accuracy） | 分类任务 | 正确数 / 总数 |
| 精确率（Precision） | 正类重要时 | TP / (TP + FP) |
| 召回率（Recall） | 漏检代价高时 | TP / (TP + FN) |
...
完善的评估体系是提示词工程质量保障的基础。核心评估维度包括正确性、一致性、相关性、完整性、安全性和效率。主要评估方法包括：

1. **基于标准答案**：适用于有明确正确答案的任务
2. **LLM-as-Judge**：适用于开放式任务的质量评估
3. **A/B 对比**：用于比较提示词版本的相对效果
4. **一致性评估**：检验输出的稳定性

建立自动化评估流水线和生产监控，可以实现提示词质量的持续保障和迭代优化。

### 12.3.9 延伸阅读

- [Judging LLM-as-a-Judge](https://arxiv.org/abs/2306.05685) - LLM 评估方法论文
- [Automatic Prompt Engineer (APE)](https://arxiv.org/abs/2211.01910) - 自动提示词优化论文
- [PromptBench](https://github.com/microsoft/promptbench) - 微软提示词评估框架
- [LangSmith](https://www.langchain.com/langsmith) - LangChain 评估与监控工具
- [Weights & Biases Prompts](https://wandb.ai/site/prompts) - 提示词版本管理与评估
