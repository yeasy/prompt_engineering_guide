...
经验估计：简单翻译通常较稳定，但仍需按目标语言和质量标准抽检
```
...
经验估计：多步推理任务通常受益于结构化过程；实际提升需用同一测试集对比
```
...
改进：RAG 往往能显著提升事实性任务表现；具体准确率应以业务评测集为准
```
...
- 对小任务考虑使用轻量模型（如 Claude Haiku、GPT-5.6 Luna）而非旗舰模型
```
