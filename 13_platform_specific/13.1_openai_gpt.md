...
> **说明（核验日期：2026-05-16）：** 本节所涉及的 API 参数、模型特性及最佳实践基于 2026 年 5 月的官方文档编写。由于 OpenAI 平台更新频繁，在实际应用中请参考最新的官方文档。

OpenAI 是大语言模型商业化的先驱，其 GPT 系列是应用最广泛的语言模型。本节深入介绍针对 GPT 系列的提示词优化策略，涵盖从 GPT-4o 到最新的 GPT-5.x 推理模型。对新项目而言，Responses API 与 Agents SDK 已经成为构建工具调用、状态管理和智能体工作流的主入口；Chat Completions 更适合作为兼容旧代码和简单对话的稳定接口。

> **注意（平台变更）**：OpenAI 官方公告称，自 **2026 年 2 月 13 日**起，GPT-4o 已从**ChatGPT**中退役；同时明确说明该变更**不影响 API**。如果你的系统强绑定某个模型版本，请以官方最新公告/文档为准，并通过回归测试验证迁移影响。
...
| 模型系列 | 核心特点 | 适用场景 |
|---------|---------|----------|
| GPT-5.5 | 最新旗舰（$5/$30 per M tokens） | 复杂分析、高精度任务、代理工作流 |
| GPT-5.4 | 1M 上下文、低幻觉、原生 Computer Use | 通用高级任务 |
| GPT-5.3 | 前沿编程能力（Codex 基础） | 编程密集型任务 |
| GPT-5.2 | 长上下文推理、电子表格/演示文稿 | 商业分析、多步执行 |
| GPT-5.1 | 高性能、增强推理（ChatGPT 侧已退役，API 仍可用） | 通用高级任务（建议迁移至 5.3/5.4） |
