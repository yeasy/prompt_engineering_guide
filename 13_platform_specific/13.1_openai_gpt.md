...
- **目的**：展示 GPT 系列中“系统提示词（system prompt）+ 用户提示词（user prompt）”的最小消息结构。
- **输入**：`system` 中描述角色、风格与输出约束；`user` 中提供具体任务与必要上下文。
- **输出**：模型遵循系统提示词的格式要求（如 Markdown、表格、结论优先）。
- **注意**（可选）：生产环境建议补充数据口径/时间范围，并用结构化输出约束减少歧义。
...
**目的**：定义工具 schema，使模型能通过函数调用请求外部能力。
**输入**：`tools` 定义 + 用户问题；可在 system 提示词中要求“需要外部信息时必须调用工具”。
**输出**：响应包含 `tool_calls`（含 `name` 与 `arguments`）；应用执行后回填 tool 结果，再由模型生成最终答复。
**注意**（可选）：服务端必须校验 `arguments`、权限与超时；不要让模型直接编造工具返回值。
