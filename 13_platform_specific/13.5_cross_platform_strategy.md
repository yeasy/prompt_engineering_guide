...
不同模型由于训练数据分布、指令微调（Instruction Tuning）策略的差异，对提示词格式的偏好也不同。以下是主流模型对提示词语法的偏好矩阵：
...
3.  **负面约束（Negative Constraints）**：不同模型对"不要做某事"的敏感度不同，Claude 通常比 GPT 更遵守安全边界。
...
对于非常复杂的提示词，手动编写适配器可能很繁琐。一种更高级的技巧是使用 **元提示词（Meta-Prompting）**，让一个强模型（如 GPT-4 或 Claude Opus）充当"编译器"，将一个通用提示词"编译"为特定模型的最佳格式。
