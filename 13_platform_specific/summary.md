...
- **预填充 (Prefilling)**：在支持的 API 中，预先写入 `Assistant` 角色的开头内容以强制模型按某种特定格式继续回答。**注：Claude Mythos Preview / Opus 4.7 / Opus 4.6 / Sonnet 4.6 已停止支持 prefill 并返回 400；Sonnet 4.5 仍是官方 Messages 文档中的 prefill 示例模型。详见 [13.2 节](13.2_anthropic_claude.md)。**
...
2. **Anthropic Claude 系列 (Claude Opus 4.7, Claude Sonnet 4.6, Claude Haiku 4.5)**
   ...
   - **预填充兼容性收窄**：早期 Claude（Sonnet 4.5 / Sonnet 4 / 3.5 及更早）可用预填充技巧（提供大括号 `{` 作为 Assistant Response 开头）控制 JSON 输出，但 Claude Mythos Preview、Opus 4.7、Opus 4.6、Sonnet 4.6 已停止支持 prefill 并返回 400。新项目请优先评估 Structured Outputs、System Prompt 或 `output_config.format`（详见 13.2 节）。
