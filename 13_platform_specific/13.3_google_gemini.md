...
Gemini 的核心优势在于原生多模态架构和业界领先的超长上下文窗口。针对 Gemini 的提示词优化应充分利用：

1. **多模态融合**：自然地混合文本、图像、音频、视频输入
2. **超长上下文**：处理整本书籍或长视频的分析任务
3. **Google 生态**：结合 Search、Workspace 等服务增强能力
4. **结构化输出**：使用 JSON Schema 确保格式一致

Gemini 特别适合需要跨模态理解和超长文档处理的复杂分析任务。

### 13.3.10 2026 年新功能

#### Personal Intelligence（个人智能）

2026 年 1 月推出的 Beta 功能，允许 Gemini 3 安全连接用户的 Google 应用：

- **跨应用数据整合**：连接 Gmail、Photos、Search 历史、YouTube 等
- **个性化响应**：基于用户数据提供更相关的建议
- **隐私控制**：默认关闭，需用户主动启用

```python
# Personal Intelligence 目前面向 Google AI Pro 和 AI Ultra 订阅用户
# 仅在美国地区可用
```

#### Veo 3.1 视频生成

Google 的视频生成模型获得重大升级：
...
- **预构建 AI Agent**：开箱即用的购物和客服 Agent
- **UCP 协议**：Universal Commerce Protocol，与 Shopify、Walmart 等共同开发的开放商务标准

### 13.3.11 延伸阅读

- [Google AI Studio](https://aistudio.google.com/) - Gemini 开发平台
- [Gemini API Documentation](https://ai.google.dev/gemini-api/docs) - 官方 API 文档
- [Multimodal Prompting Guide](https://ai.google.dev/gemini-api/docs/multimodal-prompting) - 多模态提示指南
- [Long Context Guide](https://ai.google.dev/gemini-api/docs/long-context) - 长上下文使用指南
- [Gemini Cookbook](https://github.com/google-gemini/cookbook) - 官方示例代码库
