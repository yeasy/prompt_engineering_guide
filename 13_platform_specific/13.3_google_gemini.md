...
> **说明（核验日期：2026-05-26）：** 本节内容基于 2026 年 5 月 Gemini 模型家族的官方文档编写。考虑到多模态模型的快速演进，在实际应用中请参考 Google 最新的开发者文档和 deprecations 页面。生产系统不要把 Preview 模型名称、上下文窗口、价格或关闭日期写死在业务逻辑中，应通过配置和回归测试管理升级。
...
> **上下文窗口详解**：Gemini 2.5 Pro / Flash / Flash-Lite 的官方模型页列出 1,048,576 输入 token 与 65,536 输出 token，但 deprecations 页面已公布 2026 年关闭窗口。对生产环境来说，通常优先选择官方标记为 Stable 且未进入关闭窗口的模型；`Gemini 3.1 Pro Preview` 更适合在可接受预览波动时做前沿评估。Google 官方模型页提示 Gemini 3 Pro Preview 已于 2026-03-09 关闭，应迁移到 Gemini 3.1 Pro Preview。
