...
> **说明（核验日期：2026-05-18）：** 本节内容基于 2026 年 5 月 Gemini 模型家族的官方文档编写。考虑到多模态模型的快速演进，在实际应用中请参考 Google 最新的开发者文档。生产系统不要把 Preview 模型名称、上下文窗口或价格写死在业务逻辑中，应通过配置和回归测试管理升级。
...
| 模型 | 上下文窗口 | 核心特点 | 适用场景 |
|------|-----------|---------|----------|
| Gemini 3.5 Flash | 以官方模型页为准 | 稳定版、面向持续 agent 与代码任务 | 生产候选、需要稳定可用性的多模态工作流 |
| Gemini 3.1 Pro Preview | 预览模型，限制以官方模型页为准 | 最新预览、复杂推理与 agent 工作流 | 评估最新能力、前沿试验 |
| Gemini 2.5 Pro | 1,048,576 输入 / 65,536 输出 token | 稳定旗舰、复杂推理与多模态 | 复杂分析、长文档 |
| Gemini 2.5 Flash | 1,048,576 输入 / 65,536 输出 token | 低延迟、高性价比、具备推理能力 | 实时交互、批量处理 |
| Gemini 2.5 Flash-Lite | 1,048,576 输入 / 65,536 输出 token | 成本优化、极速 | 高并发场景 |
| Gemini 1.5 Pro | 历史百万级上下文模型 | 历史兼容 | 既有系统维护 |
| Gemini Nano | 设备端 | 轻量级、离线运行 | 移动端本地应用 |

> **上下文窗口详解**：Gemini 2.5 Pro / Flash / Flash-Lite 的官方模型页列出 1,048,576 输入 token 与 65,536 输出 token。对生产环境来说，通常优先选择官方标记为 Stable 的模型；`Gemini 3.1 Pro Preview` 更适合在可接受预览波动时做前沿评估。Google 官方模型页提示 Gemini 3 Pro Preview 已于 2026-03-09 关闭，应迁移到 Gemini 3.1 Pro Preview。
...
> **注意**：以下代码示例使用稳定版 `gemini-2.5-pro`。如果你要测试最新预览能力，可以在非生产环境中替换为 `gemini-3.1-pro-preview`。请始终参考 [Google 官方文档](https://ai.google.dev/gemini-api/docs/models)获取最新的可用模型列表。
...
- Gemini 2.5 Pro（1,048,576 输入 token）：约 75 万字量级的文本、长篇技术资料、多份长文档联合分析，以及较长时长的视频/音频理解任务
