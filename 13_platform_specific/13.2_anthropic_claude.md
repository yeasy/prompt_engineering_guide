...
Claude 的长上下文能力可以处理整本书籍，但需要注意信息组织策略。Anthropic 当前模型概览列出 Fable 5、Opus 4.8 与 Sonnet 5 为 1M 上下文窗口，Haiku 4.5 为 200K；Sonnet 4.6 已移入 Legacy models 折叠区，同样是 1M。长上下文请求的价格、速率限制和 token 计数可能随模型变化，生产接入前应使用官方价格页、Models API 和 token counting API 按目标模型实测。
