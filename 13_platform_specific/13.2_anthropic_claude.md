...
> **注意**：Extended Thinking（`type: "enabled"`）在 Claude 4.6 及以上版本已弃用。建议改用 Adaptive Thinking（`type: "adaptive"`），模型将自动调整思考深度而无需手动设定 token 预算。
...
Claude 的超长上下文窗口（Sonnet 4.6/Opus 4.6 为 1M，Haiku 4.5 为 200K）可以处理整本书籍，但需要注意信息组织策略。
