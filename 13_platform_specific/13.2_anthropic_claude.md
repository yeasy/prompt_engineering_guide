...
旧版 Claude 可通过 assistant-message prefill 预设回复开头来控制格式；但 Opus 4.7、Opus 4.6、Sonnet 4.6、Sonnet 4.5 等新模型已不支持 prefill，使用时会返回 400 错误。新项目应优先使用 Structured Outputs、`output_config.format` 或系统提示词约束输出格式；只有维护旧模型或其他明确支持 assistant turn 续写的模型时，才保留 prefill 作为兼容路径。
