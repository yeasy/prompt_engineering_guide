...
> **说明（核验日期：2026-06-17）：** 本节所涉及的模型特性及最佳实践基于 2026 年 6 月的官方文档编写。在实际应用中请参考 Anthropic 最新的官方文档。
...
| 模型 | 上下文 | 核心优势 | 适用场景 |
|------|-------|---------|----------|
| Claude Fable 5 | 1M | 能力最强的广泛发布模型（2026-06-09 GA，$10/$50，Adaptive Thinking 常开，128K 输出）；曾于 2026-06-12 暂停访问，截至 2026-07-09 已恢复 | 发布规格快照；生产接入前核验可用性 |
| Claude Opus 4.8 | 1M | 最强 Opus 档能力、复杂推理、长程代理式编码 | 最复杂研究、代码库分析、高级推理 |
| Claude Sonnet 4.6 | 1M | 领先编码能力、VS Code 集成 | 企业应用、软件开发 |
| Claude Haiku 4.5 | 200K | 快速响应、低成本 | 简单任务、高并发 |

> 2026-06-09 同日发布的 Claude Mythos 5 仅面向 Project Glasswing 受邀客户提供，不对外公开发售；两者曾于 2026-06-12 被暂停访问，截至 2026-07-09 官方模型页已恢复 Fable 5 为 GA、Mythos 5 为受限可用。

> **说明（平台迁移）**：Anthropic 当前文档使用 `platform.claude.com` 作为 Claude API 文档入口。旧模型的弃用状态和迁移路径应以官方 model deprecations 页面为准。
...
Claude 可通过 assistant-message prefill 预设回复开头来控制格式；但 Claude Fable 5、Mythos 5、Mythos Preview、Opus 4.8、Opus 4.7、Opus 4.6、Sonnet 4.6 已不支持 prefill，使用时会返回 400 错误。Anthropic 官方 Messages 文档仍使用 Sonnet 4.5 展示 prefill 示例；新项目应优先使用 Structured Outputs、`output_config.format` 或系统提示词约束输出格式，只有维护明确支持 assistant turn 续写的模型时，才保留 prefill 作为兼容路径。
...
Claude 的长上下文能力可以处理整本书籍，但需要注意信息组织策略。Anthropic 当前模型概览列出 Opus 4.8 与 Sonnet 4.6 为 1M 上下文窗口，Haiku 4.5 为 200K。长上下文请求的价格、速率限制和 token 计数可能随模型变化，生产接入前应使用官方价格页、Models API 和 token counting API 按目标模型实测。
