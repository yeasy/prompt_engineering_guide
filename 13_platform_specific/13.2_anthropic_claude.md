...
预填充是 Claude 上官方推荐且效果最稳的功能（Gemini 与部分开源模型也支持类似的 assistant turn 续写），通过预设回复的开头部分精确控制输出格式。注：Opus 4.7、Opus 4.6、Sonnet 4.6、Sonnet 4.5 等新模型已不支持 prefill，使用时会返回 400 错误，请改用 Structured Outputs 或系统提示词。
