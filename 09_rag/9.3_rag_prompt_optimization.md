...
> [!TIP]
> 在生产环境中，建议将 System Prompt（角色定义和规则）放入 `system` 参数，将检索文档和用户问题放入 `messages`，以便利用[提示词缓存](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching)降低重复调用的成本。
