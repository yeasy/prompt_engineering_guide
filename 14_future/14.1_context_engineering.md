...
**[上下文工程](https://yeasy.gitbook.io/context_engineering_guide/)** 关注的不仅是提示词文本本身，而是 **整个上下文窗口的系统性设计与管理**。
...
Anthropic 的研究团队指出：“真正决定 智能体（agent） 行为的不只是提示词，而是包括系统指令、工具定义、检索内容、对话历史等在内的完整上下文。”

### 14.1.2 为什么上下文工程至关重要

最新研究表明，智能体（agent） 系统的性能高度依赖于上下文的质量。对 智能体（agent） 技能的一项广泛评估显示：
- **80% 的性能方差** 来自上下文回填的质量，即能否准确提供相关的背景知识和工具定义。
- **10% 的性能方差** 来自探索能力，即 智能体（agent） 尝试使用不同工具的策略。
- **仅 5% 的性能方差** 来自模型本身的能力差异。
...
2. **上下文中毒**
   当检索器召回了大量表面相关但实际无关的信息时，这些噪音会显著降低模型的推理能力。与其提供大量低质量的参考资料，不如提供少量高精度的信息。
