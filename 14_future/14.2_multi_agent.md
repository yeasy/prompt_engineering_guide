...
> [!WARNING]
> 共享上下文模式成本较高：每个子 Agent 启动时都需要 Prefill 大量输入，且无法复用主 Agent 的 KV 缓存。应根据任务性质灵活选择。
...
多 Agent 协作是处理复杂 AI 任务的关键架构模式。通过合理的分工设计、清晰的通信协议和专业化的提示词，可以构建出远超单一 Agent 能力的复杂系统。

主流架构包括层级式、对话式、流水线和并行模式，可根据任务特点灵活选择。LangGraph、CrewAI、AutoGen 等框架提供了丰富的编排能力，而 A2A 等协议正在推动 Agent 通信的标准化。

随着技术的发展，多 Agent 系统将变得更加自主、协作更加高效，成为下一代 AI 应用的核心架构。
