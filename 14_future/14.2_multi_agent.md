...
多 智能体（agent） 系统是 AI 应用的重要发展方向，通过让多个专业化 智能体（agent） 分工协作，可以解决单一 智能体（agent） 难以处理的复杂任务。本节深入探讨多 智能体（agent） 架构设计、协作模式和提示词策略。

### 14.2.1 为什么需要多 智能体（agent）

单一 智能体（agent） 面临的挑战：

| 挑战 | 单 智能体（agent） 局限 | 多 智能体（agent） 优势 |
|------|-------------|--------------|
| 能力边界 | 需要全能型提示词 | 专业化分工 |
...
*图 14.2-1：层级式多 智能体（agent） 架构*

**特点**：
- 中央编排 智能体（agent） 负责任务分解和结果整合
- 子 智能体（agent） 专注各自领域
- 通信清晰，便于调试
...
LangGraph 使用图结构表示 智能体（agent） 工作流：
...
Microsoft 的 AutoGen 支持灵活的多 智能体（agent） 对话：
...
Google 提出的 A2A 协议标准化了 智能体（agent） 间通信：
...
*图 14.2-4：智能体（agent） 状态共享机制*

### 14.2.5 实战协作模式

生产级多 智能体（agent） 系统通常采用以下两种通信模式：

#### 模式一：任务委托（通过通信实现隔离）

这是经典的主-子 智能体（agent） 设置，核心思想是：**主 智能体（agent） 发任务，子 智能体（agent） 交结果，中间过程免打扰**。
...
**特点**：
- 子 智能体（agent） 的上下文完全独立，从零开始
- 适用于"过程不重要，只关心结果"的任务
- 例如：代码搜索、数据查询、格式转换

从主 智能体（agent） 视角，它只是调用了一个函数，但背后实际上是另一个拥有独立工作流的子 智能体（agent） 在执行。这种模式也被称为 **"智能体（agent） 即工具"**。

#### 模式二：信息同步（通过共享上下文实现协作）

对于高度依赖历史信息的复杂任务，子 智能体（agent） 需要继承完整的上下文：
...
> **警告：** 共享上下文模式成本较高：每个子 智能体（agent） 启动时都需要 Prefill 大量输入，且无法复用主 智能体（agent） 的 KV 缓存。应根据任务性质灵活选择。

#### 智能体（agent） 化的 MapReduce

当需要从多个并行子 智能体（agent） 收集结果时，关键挑战是如何获得结构一致的输出：

**解决方案三要素**：

1. **共享沙箱**：所有 智能体（agent） 共享同一个文件系统，信息传递只需传递文件路径
2. **输出 Schema**：主 智能体（agent） 预先定义结构化的输出模式
3. **约束解码**：强制子 智能体（agent） 的输出严格符合 Schema
...
这种设计的核心思路是：**使用结构化输出作为"契约"，保证信息在 智能体（agent） 间完整、一致地传递**。
...
1. 多 智能体（agent） 协作和"一个 智能体（agent） + 多工具"相比，优势在哪里？什么程度的复杂性才值得引入多 智能体（agent） 架构？
2. 当两个 智能体（agent） 对同一问题给出矛盾的建议时，系统应该如何仲裁？你会如何在编排层面解决这个问题？
