...
传统的大语言模型（如 GPT-4, Claude Sonnet 4.6, Llama 3）采用的是 **隐式推理** 或纯基于外部提示词触发的推理。而在推理模型的训练阶段，研究者利用强化学习不仅教导模型“如何给出正确答案”，更教导模型“**如何自己构建思维链**”。
...
如果你正在使用 o1、o3、DeepSeek-R1 或 Claude 的 Adaptive Thinking（Fable 5 / Opus 4.8 / Opus 4.7 / Opus 4.6 / Sonnet 4.6），请遵循以下“少即是多”的策略：

> **注意（核验日期：2026-06-17）**：Adaptive Thinking（`type: "adaptive"`）支持 Claude Fable 5 / Mythos 5（常开，不可关闭）、Opus 4.8、Opus 4.7、Opus 4.6 与 Sonnet 4.6；Fable 5 / Mythos 5 不提供手动 Extended Thinking（曾于 2026-06-12 暂停访问，截至 2026-07-09 已恢复）；Opus 4.8 / 4.7 只支持 adaptive，不再接受手动 Extended Thinking；Opus 4.6 与 Sonnet 4.6 上的手动 Extended Thinking（`type: "enabled"` + `budget_tokens`）已被标为 deprecated。接入前请按官方文档核对目标模型支持的 `thinking` 类型和当前可用性。
...
> **注意**：此建议仅适用于 Reasoning 模型（如 o1、o3、Claude 的 Adaptive Thinking）。对于普通模型（如 GPT-4、Claude Sonnet），Chain-of-Thought 提示仍然有效且推荐使用，包括在提示词中请求“思考”或“深入思考”（参见第 6.2 节）。
