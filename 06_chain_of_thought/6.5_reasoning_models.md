...
> **注意（核验日期：2026-06-09）**：Adaptive Thinking（`type: "adaptive"`）支持 Claude Fable 5 / Mythos 5（常开，不可关闭）、Opus 4.8、Opus 4.7、Opus 4.6 与 Sonnet 4.6；Fable 5 / Mythos 5 不提供手动 Extended Thinking；Opus 4.8 / 4.7 只支持 adaptive，不再接受手动 Extended Thinking；Opus 4.6 与 Sonnet 4.6 上的手动 Extended Thinking（`type: "enabled"` + `budget_tokens`）已被标为 deprecated。接入前请按官方文档核对目标模型支持的 `thinking` 类型。
