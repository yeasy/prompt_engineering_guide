...
幻觉是大语言模型的固有挑战，主要源于其概率生成本质和训练数据局限。幻觉可分为事实性、虚构性、逻辑性和语境幻觉四种类型，严重程度因应用场景而异。

减少幻觉需要多层次的策略组合：
1. **提示词层面**：明确约束、强制引用、承认不确定性
2. **系统架构层面**：RAG 接入可靠知识源、多模型交叉验证
3. **后处理层面**：事实核查、置信度标注、人工复核

对于高风险应用（如医疗、法律、金融），应采用最严格的验证机制和人工兜底策略。理解幻觉的本质有助于设计更可靠的 AI 应用。

### 11.2.9 延伸阅读

- [Survey of Hallucination in Natural Language Generation](https://arxiv.org/abs/2202.03629) - 幻觉问题综述
- [OWASP LLM Top 10](https://owasp.org/www-project-top-10-for-large-language-model-applications/) - LLM 安全风险清单
- [Anthropic: Reducing Hallucinations](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/reduce-hallucinations) - Claude 减少幻觉指南
- [TruthfulQA Benchmark](https://github.com/sylinrl/TruthfulQA) - 幻觉检测基准测试
